...
function DashboardBreadcrumb({ pathname }: { pathname: string }) {
  const t = useT();
  const key = PAGE_KEYS[pathname] ?? (pathname.startsWith("/dashboard/tools/") ? "page.tools" as const : undefined);
  ...
  return (
    <Breadcrumb>
      <BreadcrumbList>
        <BreadcrumbItem>
          <BreadcrumbLink href="/dashboard" className="text-xs">{t("breadcrumb.dashboard")}</BreadcrumbLink>
        </BreadcrumbItem>
        <BreadcrumbSeparator />
        <BreadcrumbItem>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-12 shrink-0 items-center justify-between border-b border-border px-4 lg:px-6">
          <div className="flex items-center gap-3">
            <button
              className="lg:hidden flex size-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-accent/50 hover:text-foreground transition-colors"
