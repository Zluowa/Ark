...
const BILIBILI_REFERER = 'Referer:https://www.bilibili.com'
const GENERIC_DESKTOP_USER_AGENT =
...
const appendPlatformNetworkSafetyArgs = (args: string[], url: string): void => {
  if (isYouTubeUrl(url) || isBilibiliUrl(url)) {
    args.push('--force-ipv4')
  }

  if (isBilibiliUrl(url)) {
    args.push('--add-header', BILIBILI_REFERER)
    args.push('--add-header', GENERIC_DESKTOP_USER_AGENT)
  }
}
...
export const buildDownloadArgs = (
  ...
  appendPlatformNetworkSafetyArgs(args, options.url)
  ...
  if (options.type === 'video') {
    ...
    if (trim(options.audioFormat)) {
      args.push('--audio-format', trim(options.audioFormat))
    }
  }
...
export const buildVideoInfoArgs = (
  ...
  appendPlatformNetworkSafetyArgs(args, url)
