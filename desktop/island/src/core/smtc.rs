...
impl SmtcListener {
    ...
    fn init(&self) {
        ...
        std::thread::spawn(move || {
            ...
            let update_info = |mgr: &GlobalSystemMediaTransportControlsSessionManager,
                               arc: &Arc<Mutex<MediaInfo>>| {
            ...
            let handler = TypedEventHandler::new(
                move |m: &Option<GlobalSystemMediaTransportControlsSessionManager>, _| {
                    if let Some(mgr) = m {
    ...
    fn fetch_properties(
        ...
        if let Ok(mut info) = info_arc.lock() {
            ...
            if song_changed
                || (position_ms as i64 - extrapolated as i64).abs() > 1500
                ...
                if let Ok(stream_async) = thumb_ref.OpenReadAsync() {
                    if let Ok(stream) = stream_async.get() {
                        if let Ok(size) = stream.Size() {
                            let buffer =
                                windows::Storage::Streams::Buffer::Create(size as u32).unwrap();
                            if let Ok(res_buffer_async) = stream.ReadAsync(
                                &buffer,
                                size as u32,
