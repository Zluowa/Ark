// @input: MediaInfo, canvas, text/lyric strings
// @output: Shared rendering utilities: text cache, media image, palette, visualizer
// @position: Common draw helpers used by render.rs; replaces expanded/main_view.rs public API

use crate::core::persistence::load_config;
use crate::core::smtc::MediaInfo;
use skia_safe::{
    gradient_shader, Canvas, Color, Data, Font, FontMgr, FontStyle, Image, Paint, Point, Rect,
    TileMode, Typeface,
};
...
pub fn draw_text_cached(
    ...
    TEXT_CACHE.with(|cache| {
        ...
        if !cache_mut.contains_key(&cache_key) {
            ...
            let mut groups: Vec<(String, Typeface)> = Vec::new();
        ...
        let mut x = if align_center {
            pos.0 - total_width / 2.0
        } else {
            pos.0
        };
        let y = pos.1.round();
    ...
    COLOR_CACHE.with(|cache| {
        ...
        if img.read_pixels(
            ...
            if count > 0 {
                ...
                palette.push(brighten(r_avg, g_avg, b_avg, 1.3));
                palette.push(brighten(r_avg, g_avg, b_avg, 1.8));
                palette.push(brighten(r_avg, g_avg, b_avg, 1.3));
            }
        }
        if palette.is_empty() {
