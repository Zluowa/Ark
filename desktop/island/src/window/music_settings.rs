...
impl MusicApp {
    ...
    fn draw(&mut self) {
        ...
        canvas.draw_round_rect(
            Rect::from_xywh(20.0, 70.0, MUSIC_W - 40.0, 100.0),
            12.0,
            12.0,
            &paint,
        );
        ...
        if self.detected_apps.is_empty() {
            ...
            for app in &self.detected_apps {
                paint.set_color(COLOR_CARD);
                canvas.draw_round_rect(
                    Rect::from_xywh(20.0, current_y, MUSIC_W - 40.0, 45.0),
                    10.0,
                    10.0,
                    &paint,
                );
                let is_active = self.config.smtc_apps.contains(app);
                paint.set_color(if is_active && enabled {
                    COLOR_ACCENT
                } else {
                    if enabled {
                        COLOR_TEXT_SEC
                ...
                if enabled {
                    ...
                    canvas.draw_str(
                        "Delete",
                        (335.0, current_y + 27.0),
                        &self.get_font(12.0, false),
                        &paint,
                    );
                }
                current_y += 50.0;
                if current_y > MUSIC_H - 50.0 {
                    break;
                }
            }
        }
...
impl ApplicationHandler for MusicApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        surface
            .resize(
                std::num::NonZeroU32::new((MUSIC_W as f64 * scale) as u32).unwrap(),
                std::num::NonZeroU32::new((MUSIC_H as f64 * scale) as u32).unwrap(),
            )
            .unwrap();
